...
def load_vocab():
    ...
    for (term, idf_value) in zip(vocab_terms, idf_values):
        vocab1[term.rstrip()] = int(idf_value.rstrip())
...
def load_inverted_index():
    ...
    for row_num in range(0, len(inverted_index_terms), 2):
        ...
        inverted_index1[term] = documents
...
def load_link_of_qs():
    with open("qData/Qlink.txt", "r",encoding = 'utf-8', errors = "ignore") as f:
        links = f.readlines()
...
def calc_docs_sorted_order(q_terms):
    # will store the doc which can be our ans:
    # sum of tf-idf value of that doc for all the query terms
    potential_docs = {}
    ans = []
    for term in q_terms:
